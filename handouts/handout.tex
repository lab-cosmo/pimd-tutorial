...
when calling the software (see section 3.6 of PWscf user's
manual).\newline
...
\texttt{<address>} node we have to provide a \texttt{<port>} node that
would be ignored when a ``UNIX'' socket is used.
...
then replaced by the \texttt{UNIX} string that tells \pwx\ to use a UNIX socket.
