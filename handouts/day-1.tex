...
%
Please note that \ipi{} can read the unit cell from the comment line in the xyz file. %, and this should override the unit cell specified in the cp2k input file.
%
The cell and positions in the cp2k input file is just for the purpose of initializating the cp2k program and at the first simulation step both would be overridden.
%Also, one can specify the unit cell in the \ipi{} input file, which will override the unit cell read in from the xyz file.
...
\lstinxml{mode="langevin"} specifies the type of thermostat in use,
...
\Question
Executing \url{extract-gOH.bash} and \url{extract-kin.bash} will create a file called gOH.data inside each of the sub directories \lstinbash{n.*}.
\begin{lstlisting}[language=bash]
$ pwd
